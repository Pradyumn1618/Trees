...
struct node* rotate_left(struct node* root){
    ...
    bool col=root->col;
       ...
       root->right->col=col;
    if(root->p!=NULL){
        if(root->p->left==root){root->p->left=right;right->p=root->p;}
        else if(root->p->right==root){root->p->right=right;right->p=root->p;}
...
struct node* rotate_right(struct node* root){
    ...
    bool col=root->col;
       ...
       root->left->col=col;
    if(root->p!=NULL){
        if(root->p->left==root){root->p->left=left;left->p=root->p;}
        else if(root->p->right==root){root->p->right=left;left->p=root->p;}
...
struct node* successor(struct node* root,int val){
    ...
    if(temp==NULL){
        ...
        while(root->data!=val){
      if(root->data>val){min=root;root=root->left;}
      else if(root->data<val)root=root->right;
       }
        if(min->data>val)
        return min;
...
struct node* delete(struct node* root,int val){
   ...
   if(z->left==NULL&&z->right==NULL&&z->col==1){
    if(z==z->p->left){z->p->left=NULL;}              
    else{z->p->right=NULL;}                     //red leaf node
    return tree_root;
    }
...
//Cases for deleting black leaf node
   ...
   if(z->left==NULL&&z->right==NULL){
    struct node* s;
    if(z==z->p->left){s=z->p->right;z->p->left=NULL;}
    else{s=z->p->left;z->p->right=NULL;}
    if(s==NULL)x=z->p;
    else{
    ...
    x=z->right;
    if(z->p!=NULL){
        if(z->p->right==z){z->p->right=x;}
        else{z->p->left=x;if(x!=NULL)x->p=z->p;}
    }
    else{tree_root=x;}
    if(x!=NULL)
   ...
   if(y_ori_col==0&&x!=NULL)correct_tree2(x);
   return tree_root;
}

void correct_tree(struct node* root){
       if(root==NULL)return;
       if(root->col==0)return;
       if(root->p==NULL){root->col=0;return;}
       else if(root->p->p==NULL)return;
       else if(root->p!=NULL&&root->p->col!=0){
        struct node* u;
        if(root->p->p->left==root->p){
            u=root->p->p->right;
        }
        else u=root->p->p->left;
        ...
        if(u!=NULL&&u->col==1){
            ...
            if(root->data<=root->p->data&&root->p->data<=root->p->p->data)rotate_right(root->p->p);
            else if(root->data>root->p->data&&root->p->data>root->p->p->data)rotate_left(root->p->p);
            else if(root->data<=root->p->data&&root->p->data>root->p->p->data){root->p->p->right=rotate_right(root->p);rotate_left(root->p);}
            else if(root->data>root->p->data&&root->p->data<=root->p->p->data){root->p->p->left=rotate_left(root->p);rotate_right(root->p);}
            root=root->p;
        }
          correct_tree(root);
...
struct node* insert(struct node* root,int key){
    ...
    if(temp!=NULL){printf("%d is already present.So no need to insert.\n",key);return NULL;}
    struct node* new=createnode(key);
    while(root!=NULL){
        if(key<=root->data&&root->left!=NULL)root=root->left;
        else if(key>root->data&&root->right!=NULL)root=root->right;
        else if(key<=root->data){root->left=new;new->p=root;break;}
        else if(key>root->data){root->right=new;new->p=root;break;}
    }
    correct_tree(new);
    return new;
}
...
void enqueue(struct node* a[],struct node* c){
    if(tail>99){printf("Queue overflow!\n");exit(-1);}
    a[tail]=c;
    tail++;
        ...
        // if(d==1||d==2||d==3)scanf("%d",&e);
           if(d==1){
            ...
            scanf("%d",&e);
              ...
              if(ins!=NULL)printf("%d inserted\n",e);
           }
           else if(d==3){
            printf("Enter the number to be deleted: ");
            scanf("%d",&e);
              struct node* del=delete(tree_root,e);
              if(del!=NULL)printf("%d deleted!\n",e);
           }
           else if(d==4){
