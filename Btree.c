...
struct BTreeNode* createnode(int val){
    struct BTreeNode* new = (struct BTreeNode*)malloc(sizeof(struct BTreeNode));
     new->key[0]=val;
     for(int i=0;i<MAX+1;i++)new->children[i]=NULL;
     new->count=1;
     return new;
}
...
void split(struct btn* root,struct btn* parent,int b){
    ...
    for(int i=0;i<MAX+1;i++)new_root->children[i]=NULL;
    new_root->count=0;
...
void insert(struct BTreeNode* root,int val){
    ...
    if(sea!=NULL){printf("%d is already present. So no need to insert.\n",val);flag=false;return;}
    if(root->count==MAX){split(root,NULL,1);insert(tree_root,val);}
    else{
        int i;
        for(i=0;i<root->count;i++){
            if(val>root->key[i])continue;
...
int successor(struct btn* root){
   if(root->children[0]==NULL)return root->key[0];
   else return successor(root->children[0]);
}

void replace(struct btn* root,int a,int b){
    int i;
   for(i=0;i<root->count;i++){
        if(a>root->key[i])continue;
   ...
   if(i==root->count){root=root->children[i];replace(root,a,b);return;}
}
    ...
    for(i=0;i<root->count;i++){
        if(val>root->key[i])continue;
            ...
            if(root->children[i]!=NULL&&root->children[i]->count>MIN){
                root=root->children[i];delete(root,val);
                return;
                }
                ...
                if(i-1>=0&&root->children[i-1]->count>MIN){
                    ...
                    delete(tree_root,val);
                     ...
                     root->count--;
                     if(root->count==0)tree_root=root->children[0];
                     delete(tree_root,val);
                     return;
                }
            }
            else if(root->children[i]==NULL){printf("%d not found! Can't be deleted!\n",val);flag=false;return;}
        }
        else{
           if(root->children[i]==NULL){
            for(int j=i;j<root->count-1;j++){
...
void print_level(struct btn* root){
    ...
    while(!check_empty(queue)){
        ...
        while(cnt>0){
            ...
            dequeue(queue);
            for(int i=0;i<=temp->count;i++){
                if(temp->children[i]!=NULL)enqueue(queue,temp->children[i]);
            }
            cnt--;
        }
        lvl++;
